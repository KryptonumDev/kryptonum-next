...
const CardGrid = ({ data: { list }, additionalStyles }) => {
  return (
    <div className={additionalStyles ? `${additionalStyles.cardGrid} ${styles.wrapper} ` : styles.wrapper}>
      {list.map(({ title, description, icon, overflowContent }, i) => (
        <div className={additionalStyles ? `${additionalStyles.item} ${styles.item}` : styles.item} key={i}>
          {icon ? (
            <Img
              data={icon}
              className={additionalStyles ? `${additionalStyles.icon} ${styles.icon}` : styles.icon}
              sizes="60px"
            />
          ) : (
            <Markdown
