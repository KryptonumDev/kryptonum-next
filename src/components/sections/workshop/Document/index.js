...
const Document = ({ data: { document_Heading, document_Paragraph, document_Paragraph2, document_Images } }) => {
  return (
        ...
        <div className={styles.sliderWrapper}>
          {document_Images.map((img, i) => (
            <div className={styles.item} key={i}>
              <Img data={img} className={styles.img} sizes="(max-width: 999px) 100vw, 75vw" />
            </div>
          ))}
        </div>
      </div>
