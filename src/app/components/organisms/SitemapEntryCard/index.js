...
const EntryCard = ({ data }) => {
  return (
    <div className={styles.entry}>
      <Link href={`/pl/blog/${data.slug.current}`} className={styles.link} aria-label={removeMarkdown(data.title)} />
      <Img
      data={data.img}
      ...
      <div>
        <div className={styles.flex}>
          <Link href={`/pl/zespol/${data.author[0].slug.current}`} className={styles.author}>
            <Img
